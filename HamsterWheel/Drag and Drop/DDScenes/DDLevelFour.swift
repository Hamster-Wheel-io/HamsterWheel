...
//  HamsterWheel
//
//  Created by Phyllis Wong on 2/11/18.
...
class DDLevelFour: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        wrongMatch = childNode(withName: "wrongMatch") as! SKSpriteNode!
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let wrongMatchUpperX = wrongMatch.position.x + 90
        let wrongMatchUppery = wrongMatch.position.y + 90
        let wrongMatchLowerX = wrongMatch.position.x - 90
        let wrongMatchLowerY = wrongMatch.position.y - 90
        ...
        if wrongMatchLowerX <= xCoord && xCoord <= wrongMatchUpperX {
            if wrongMatchLowerY <= yCoord && yCoord <= wrongMatchUppery {
                print("You fucked up yo!")
                
                let vibrationAction = SKAction.run {
                    self.vibratePhone()
                }
                
                player.run(vibrationAction)
            }
        }
    }
